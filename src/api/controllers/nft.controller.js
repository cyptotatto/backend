...
export const getNftKeywords = async (req, res) => {
  try {
    ...
    if (tattooDesign == 'true') searchKeyWords.push({ tattooDesign: true });
    else searchKeyWords.push({ tattooDesign: false });
    if (req.query.genre) searchKeyWords.push({ genre: req.query.genre });
    if (req.query.theme) searchKeyWords.push({ theme: req.query.theme });
    if (req.query.part) searchKeyWords.push({ part: req.query.part });
